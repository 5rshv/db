package skypro.hogwarts.WebMvcTest;
...
import skypro.hogwarts.controller.FacultyController;
...
public class FacultyControllerTest {
    ...
    void testGetAllFaculties() throws Exception {
        List<Faculty> faculties = List.of(new Faculty(6L, "logitech", "blue"));
        when(facultyService.getAllFaculties()).thenReturn(faculties);

        mockMvc.perform(get("/faculty/all"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].name").value("logitech"))
